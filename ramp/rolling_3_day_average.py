...
class Rolling3DayAverage():
    def __init__(self) -> None:
        """I was not able to fork the dataset so I just copied it here and created a dataframe.
        The dataframe with the transaction time and transaction amount from the question."""
        self.df = pd.DataFrame(
            ...
            columns=["transaction_time", "transaction_amount"]
        )

    def clean(self) -> None:
      """Prepare the data to run to be used. Wouldn't need this if connected to DB or fork worked."""
      self.df['transaction_time'] = pd.to_datetime(self.df['transaction_time']).dt.date
      self.df['transaction_amount'] = self.df['transaction_amount'].astype(float)
      self.df = self.df.sort_values(by='transaction_time').reset_index(drop=True)

    def calculate_3_day_average(self) -> None:
        """Pandas way of simply calculating the 3 day average for a series"""
        self.df['rolling_3_day_average'] = self.df['transaction_amount'].rolling(window=3).mean()

    def get_final_3_day_average(self) -> float:
        """Retrive the final day's rolling 3 day average"""
        return self.df.tail(1)['rolling_3_day_average'].item()

    def execute(self):
       """Wrapper calling all functions to answer question"""
       self.clean()
       self.calculate_3_day_average()
       print(self.get_final_3_day_average())

def main():
    """Calling function for code"""
    question = Rolling3DayAverage()
    question.execute()
